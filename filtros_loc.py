...
busqueda_nominal = df_nombres.loc['Elena Marín', :]
...
indices_pares = [i for i in df.index if i % 2 == 0]

muestreo_intercalado = df.loc[indices_pares, ['Nombre', 'Nivel_Estudio']]

veterania = df.loc[df['Edad'] > 40, ['Nombre', 'Remoto']]

ultimas_etiquetas = df.index[-5:]

cierre_lista = df.loc[ultimas_etiquetas, :]
